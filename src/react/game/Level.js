	...
	*/ 
	get filterTiles(){
		...
		return reversed.map((tile, id) => {
			switch(tile){
				case WHITE_TILE.id:
					return <img key={id} id={"white-" + id} className="Tile" src={WHITE_TILE.svg} onClick={(e) => {this.levelStore.handleClick(e)}} alt="white" />;
				case BLACK_TILE.id:
					return <img key={id} id={"black-" + id} className="Tile" src={BLACK_TILE.svg} onClick={(e) => {this.levelStore.handleClick(e)}} alt="black" />;
				default: //Yellow tile
					return <img key={id} id={"yellow-" + id} className="Tile" src={YELLOW_TILE.svg} onClick={(e) => {this.levelStore.handleClick(e)}} alt="yellow" />;
			}
		});
	}
